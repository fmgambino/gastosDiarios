...
function route_(payload) {
  try {
    ...
    switch (action) {
      case "health":
        return json_({ ok: true, app: "Gastos Diarios", timestamp: new Date().toISOString() });
...
function delete_(payload) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]) === id) {
      sheet.deleteRow(i + 1);
      return json_({ ok: true, message: "Registro eliminado." });
    }
  }
